...
export type ValidationFailureClass = "hard" | "soft" | "auto-repair";

export interface ValidationFailure {
  message: string;
  category: ValidationFailureCategory;
  classification: ValidationFailureClass;
  source: "schema" | "matcher";
  repairHint?: string;
}

export interface RepairFallbackDecision {
  classification: ValidationFailureClass;
  action:
    | "reject"
    | "accept-with-warnings"
    | "attempt-auto-repair-and-retry";
  reason: string;
}
...
export interface ValidationReport {
  ...
  decision: RepairFallbackDecision | null;
}
...
function classifyFailure(
  ...
  if (source === "matcher") {
    return {
      ...
      category: "matcher",
      classification: "soft",
      repairHint: "Adjust prompt instructions/examples for matcher alignment.",
    };
  }

  if (
    message.includes("must have one of") ||
    message.includes("must have exactly one property in 'component'")
  ) {
    return {
      message,
      source,
      category: "protocol-envelope",
      classification: "hard",
      repairHint: "Return exactly one valid top-level message envelope.",
    };
  }

  if (
    message.includes("missing required property") ||
    message.includes("must have exactly one key from")
  ) {
    return {
      message,
      source,
      ...
      repairHint: "Inject required defaults and normalize bound-value shape.",
    };
  }

  if (message.includes("references non-existent component ID")) {
    return {
      message,
      source,
      category: "component-reference",
      classification: "hard",
      repairHint: "Create missing component ID or update reference target.",
    };
  }
  ...
  if (message.includes("Unknown component type")) {
    return {
      ...
      classification: "hard",
      repairHint: "Use only components from the supported v0.8 catalog.",
    };
  }

  if (
    message.includes("unexpected property") ||
    message.includes("Duplicate component ID")
  ) {
    return {
      message,
      source,
      ...
      classification: "soft",
      repairHint: "Drop unsupported fields and de-duplicate IDs.",
    };
  }
  ...
  return {
    ...
    category: "schema-structure",
    classification: "hard",
    repairHint: "Regenerate payload with strict schema compliance.",
  };
}

export function decideRepairFallback(
  failures: ValidationFailure[]
): RepairFallbackDecision | null {
  if (failures.length === 0) {
    return null;
  }

  if (failures.some((f) => f.classification === "hard")) {
    return {
      classification: "hard",
      action: "reject",
      reason: "One or more hard validation failures cannot be auto-repaired.",
    };
  }

  if (failures.some((f) => f.classification === "auto-repair")) {
    return {
      classification: "auto-repair",
      action: "attempt-auto-repair-and-retry",
      reason: "Failures are localized and suitable for deterministic repair.",
    };
  }
  ...
  return {
    ...
    reason: "Only soft validation failures detected.",
  };
}

export function validateSchemaWithClassification(
  data: any,
  schemaName: string,
  ...
  if (data.surfaceUpdate) {
    validateSurfaceUpdate(data.surfaceUpdate, schemaErrors);
  } else if (data.dataModelUpdate) {
    validateDataModelUpdate(data.dataModelUpdate, schemaErrors);
  } else if (data.beginRendering) {
    validateBeginRendering(data.beginRendering, schemaErrors);
  } else if (data.deleteSurface) {
    validateDeleteSurface(data.deleteSurface, schemaErrors);
  } else {
    schemaErrors.push(
  ...
  if (matchers) {
    for (const matcher of matchers) {
      ...
      if (!result.success) {
        matcherErrors.push(result.error!);
      }
    }
  }

  const errors = [...schemaErrors, ...matcherErrors];
  const failures: ValidationFailure[] = [
    ...schemaErrors.map((error) => classifyFailure(error, "schema")),
    ...matcherErrors.map((error) => classifyFailure(error, "matcher")),
  ];

  return {
    errors,
    failures,
    decision: decideRepairFallback(failures),
  };
}
...
export function validateSchema(
  ...
  return validateSchemaWithClassification(data, schemaName, matchers).errors;
}
