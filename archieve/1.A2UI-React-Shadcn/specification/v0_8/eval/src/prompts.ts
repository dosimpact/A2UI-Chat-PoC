...
export type CatalogInjectionStrategy =
  | "none"
  | "component-list"
  | "catalog-json";

export interface CatalogInjectionHooks {
  beforeCatalogInjection?: string;
  afterCatalogInjection?: string;
}
...
export interface PromptArchitecture {
  ...
  examples?: string[];
  catalogInjection?: {
    strategy: CatalogInjectionStrategy;
    hooks?: CatalogInjectionHooks;
  };
}
...
const DEFAULT_PROMPT_ARCHITECTURE = {
  system:
    "You are an A2UI protocol generator. Produce valid protocol payloads only.",
  catalogInjection: {
    strategy: "none",
  },
} as const;

function resolvePromptArchitecture(prompt: TestPrompt): PromptArchitecture {
  return {
    system: prompt.architecture?.system ?? DEFAULT_PROMPT_ARCHITECTURE.system,
    task: prompt.architecture?.task ?? prompt.promptText,
    examples: prompt.architecture?.examples,
    catalogInjection:
      prompt.architecture?.catalogInjection ??
      DEFAULT_PROMPT_ARCHITECTURE.catalogInjection,
  };
}
...
function buildCatalogSection(
  ...
  const injection = architecture.catalogInjection;
  if (!injection || injection.strategy === "none") {
    return null;
  }

  const lines: string[] = [];
  if (injection.hooks?.beforeCatalogInjection) {
    lines.push(injection.hooks.beforeCatalogInjection);
  }

  if (injection.strategy === "component-list") {
    const names = options.catalogComponentNames ?? [];
    if (names.length > 0) {
      lines.push(`Available components: ${names.join(", ")}`);
    } else {
      lines.push("Available components: (not provided for this run)");
    }
  } else if (injection.strategy === "catalog-json") {
    if (options.catalog !== undefined) {
      lines.push(`Catalog JSON: ${JSON.stringify(options.catalog)}`);
    } else {
      lines.push("Catalog JSON: (not provided for this run)");
    }
  }

  if (injection.hooks?.afterCatalogInjection) {
    lines.push(injection.hooks.afterCatalogInjection);
  }
...
export function buildPromptText(
  ...
  const architecture = resolvePromptArchitecture(prompt);
  const sections: string[] = [
    `[SYSTEM]\n${architecture.system}`,
    `[TASK]\n${architecture.task}`,
  ];

  if (architecture.examples && architecture.examples.length > 0) {
    const lines = architecture.examples.map((example, index) => {
      return `${index + 1}. ${example}`;
    });
    sections.push(`[EXAMPLES]\n${lines.join("\n")}`);
  }

  const catalogSection = buildCatalogSection(architecture, options);
  if (catalogSection) {
    sections.push(`[CATALOG]\n${catalogSection}`);
  }

  const schemaForRun = options.schemaPathOverride ?? prompt.schemaPath;
  sections.push(`[SCHEMA]\nUse schema file: ${schemaForRun}`);
