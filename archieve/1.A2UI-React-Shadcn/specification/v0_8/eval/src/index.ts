...
import { buildPromptText, prompts, TestPrompt } from "./prompts";
import {
  type RepairFallbackDecision,
  type ValidationFailure,
  validateSchemaWithClassification,
} from "./validator";
...
interface InferenceResult {
  ...
  repairDecision: RepairFallbackDecision | null;
  runNumber: number;
}
...
function generateSummary(
  ...
  const repairClassCounts = results.reduce(
    (acc, result) => {
      const classification = result.repairDecision?.classification;
      if (classification) {
        acc[classification]++;
      }
      return acc;
    },
    { hard: 0, soft: 0, "auto-repair": 0 } as Record<
      "hard" | "soft" | "auto-repair",
      number
    >,
  );

  summary += `\n- **Repair/Fallback decisions:** hard=${repairClassCounts.hard}, soft=${repairClassCounts.soft}, auto-repair=${repairClassCounts["auto-repair"]}`;
  return summary;
}
          ...
          componentGeneratorFlow({
            ...
            .then((component) => {
              if (modelOutputDir) {
                ...
                fs.writeFileSync(inputPath, promptText);
              ...
              const validationReport = validateSchemaWithClassification(
                component,
                prompt.schemaPath,
                prompt.matchers
              );
              return {
                ...
                validationResults: validationReport.errors,
                validationFailures: validationReport.failures,
                repairDecision: validationReport.decision,
                runNumber: i,
              };
            })
            .catch((error) => {
        ...
        if (hasError) {
          ...
          if (hasValidationFailures) {
            ...
            result.validationFailures.forEach((failure) => {
              console.log(
                `- [${failure.classification}/${failure.category}] ${failure.message}`
              );
            });

            if (result.repairDecision) {
              console.log(
                `Repair/Fallback Decision: ${result.repairDecision.classification} -> ${result.repairDecision.action} (${result.repairDecision.reason})`
              );
            }
          }
          if (verbose) {
