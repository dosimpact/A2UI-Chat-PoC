...
export type V08LockMode = "reject" | "flag";

export interface V08LockIssue {
  path: string;
  version: "0.9" | "0.10";
  value: string;
  mode: V08LockMode;
...
export interface V08LockAssessment {
  supported: boolean;
  shouldReject: boolean;
  issues: V08LockIssue[];
}

const UNSUPPORTED_PROTOCOL_PATTERNS: Array<{
  version: "0.9" | "0.10";
  pattern: RegExp;
}> = [
  {
    version: "0.9",
    pattern: /(^|[^a-z0-9])v?0[._-]?9([^a-z0-9]|$)/i,
  },
  {
    version: "0.10",
    pattern: /(^|[^a-z0-9])v?0[._-]?10([^a-z0-9]|$)/i,
  },
];

function getUnsupportedProtocolVersion(value: string): "0.9" | "0.10" | null {
  const normalized = value.toLowerCase();
  for (const { version, pattern } of UNSUPPORTED_PROTOCOL_PATTERNS) {
...
function inspectForUnsupportedProtocolVersions(
  value: unknown,
  path: string,
  mode: V08LockMode,
  issues: V08LockIssue[],
  ...
  if (typeof value === "string") {
    const version = getUnsupportedProtocolVersion(value);
    if (version) {
      issues.push({
        path,
        version,
        value,
        mode,
        message: `Detected unsupported protocol version '${version}' at '${path}'. v0.8 runtime supports only v0.8 payloads.`,
      });
    }
    return;
  }
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => {
      inspectForUnsupportedProtocolVersions(
        item,
        `${path}[${index}]`,
        mode,
        issues,
...
export function assessV08Lock(
  payload: unknown,
  mode: V08LockMode = "reject"
): V08LockAssessment {
  const issues: V08LockIssue[] = [];
  inspectForUnsupportedProtocolVersions(payload, "$", mode, issues, new Set());
  const shouldReject = issues.length > 0 && mode === "reject";
  return {
    supported: !shouldReject,
    shouldReject,
    issues,
  };
}
