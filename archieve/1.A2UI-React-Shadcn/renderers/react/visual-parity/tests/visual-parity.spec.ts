...
const DEFAULT_MAX_DIFF_PERCENT = 1; // 1% of pixels can differ

const RELAXED_THRESHOLDS: Partial<Record<FixtureName, number>> = {
  // Native time/video controls render slightly differently across renderers/browsers.
  dateTimeInputTime: 2.5,
  videoBasic: 2.5,
  videoWithPathBinding: 2.5,
};

function maxDiffForFixture(fixture: FixtureName): number {
  return RELAXED_THRESHOLDS[fixture] ?? DEFAULT_MAX_DIFF_PERCENT;
}
...
const skippedFixtures: FixtureName[] = [
  // Lit and React intentionally diverge here (dropdown/chips interaction model).
  // Track this separately from strict visual parity.
  'multipleChoice',
];
      ...
      for (const fixture of fixturesToTest) {
        test(`${fixture}`, async ({ browser }) => {
          ...
          try {
            ...
            const maxDiffPercent = maxDiffForFixture(fixture);
            console.log(
            ...
            ).toBeLessThanOrEqual(maxDiffPercent);
          } finally {
