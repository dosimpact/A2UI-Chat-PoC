...
import { validateAndApplyFallback } from "./validation-fallback";
                ...
                if (payload.clientUiCapabilities || payload.userAction) {
                  if (payload.clientUiCapabilities) {
                    if ("dynamicCatalog" in payload.clientUiCapabilities) {
                      const lockAssessment = v0_8.Data.Guards.assessV08Lock(
                        payload.clientUiCapabilities.dynamicCatalog,
                        "reject"
                      );
                      if (lockAssessment.shouldReject) {
                        res.statusCode = 400;
                        res.setHeader("Content-Type", "application/json");
                        res.end(
                          JSON.stringify({
                            error:
                              "Unsupported protocol version detected in dynamic catalog. v0.8 middleware accepts only v0.8 contracts.",
                            details: lockAssessment.issues,
                          })
                        );
                        return;
                      }
                  ...
                  if (v0_8.Data.Guards.isObject(payload.request)) {
                    ...
                    const fallbackResult = validateAndApplyFallback(
                      modelResponse.text ?? ""
                    );
                    if (
                      fallbackResult.decision.classification === "hard" ||
                      !fallbackResult.text
                    ) {
                      res.statusCode = 422;
                      res.setHeader("Content-Type", "application/json");
                      res.end(
                        JSON.stringify({
                          error:
                            "Model response failed hard validation checks and was rejected.",
                    ...
                    res.setHeader("Content-Type", "application/json");
                    res.setHeader(
                      "x-a2ui-validation-classification",
                      fallbackResult.decision.classification
                    );
                    res.end(
                      JSON.stringify({
                        role: "model",
                        parts: [{ text: fallbackResult.text }],
                        validation: {
                          classification:
                            fallbackResult.decision.classification,
                          action: fallbackResult.decision.action,
                          reason: fallbackResult.decision.reason,
                          failures: fallbackResult.failures,
                        },
                      })
                    );
                  } else {
